...
model_dict = pickle.load(open('model_ASL1.pkl', 'rb'))
model = model_dict['model']

# Initialize video capture with backend (important for Windows)
cap = cv.VideoCapture(0, cv.CAP_DSHOW)  # Use cv.CAP_DSHOW to avoid DirectShow conflicts on Windows
time.sleep(2)  # Delay to ensure camera initialization

# Check if camera is opened successfully
if not cap.isOpened():
    print("Error: Could not access the camera. Please check your camera connection or permissions.")
    exit()
...
labels_dict = {
    ...
    "space": " ",  # Add mapping for space gesture
    "nothing": ""  # Add mapping for no recognized gesture
}
...
while True:
    ...
    if results.multi_hand_landmarks:
        ...
        try:
            ...
            predicted_character = labels_dict.get(str(prediction[0]), "Unknown")  # Handle unknown predictions
